...
k=[1800,1200,600,600,600] #enter floor stiffness from bottom to top [k1,k2,k3...]
m=[2.5,2,1.5,1,1]         #enter floor mass from bottom to top [m1,m2,m3...]
...
for t in loops:
    ...
    axs[3].cla()
    A4=new_phi4*np.sin(cir_fr4*t)
    axs[3].scatter(A4-0.5,floors,s=50,c='r')
    axs[3].scatter(A4+0.5,floors,s=50,c='r')
    for i in range(len(k)):
        axs[3].plot([A4[i]-0.5,A4[i+1]-0.5],[floors[i],floors[i+1]],c='k')
        axs[3].plot([A4[i+1]-0.5,A4[i+1]+0.5],[floors[i+1],floors[i+1]],c='k')
        axs[3].plot([A4[i]+0.5,A4[i+1]+0.5],[floors[i],floors[i+1]],c='k')
    axs[3].set_xlim(-2,2)
    axs[3].set_ylim(0,max(floors)+1)
    plt.tight_layout()
    
    axs[4].cla()
    A5=new_phi5*np.sin(cir_fr5*t)
    axs[4].scatter(A5-0.5,floors,s=50,c='r')
    axs[4].scatter(A5+0.5,floors,s=50,c='r')
    for i in range(len(k)):
        axs[4].plot([A5[i]-0.5,A5[i+1]-0.5],[floors[i],floors[i+1]],c='k')
        axs[4].plot([A5[i+1]-0.5,A5[i+1]+0.5],[floors[i+1],floors[i+1]],c='k')
        axs[4].plot([A5[i]+0.5,A5[i+1]+0.5],[floors[i],floors[i+1]],c='k')
    axs[4].set_xlim(-2,2)
    axs[4].set_ylim(0,max(floors)+1)
    plt.tight_layout()
